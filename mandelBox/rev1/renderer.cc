...
void renderFractal(const CameraParams &camera_params, const RenderParams &renderer_params, 
		   unsigned char* image, double &testDist)
{
  ...
  int i,j,k;
  int targetPixX = height/2;
  int targetPixY = width/2;
  double dis = 0.0;

#pragma omp parallel for private(i,j,k,color,to,farPoint,pix_data) shared(image,from,time) num_threads(8)
  for(j = 0; j < height; j++)
    {
      for(i = 0; i <width; i++)
	{
	  ...
	  pSub(to, farPoint, camera_params.camPos);
	  NORMALIZE(to);
	  
	  //render the pixel
	  rayMarch(renderer_params, from, to, eps, pix_data);
	  if(i == targetPixX || i == targetPixX+1)
	    if(j == targetPixY){
	      dis+=pix_data.dist;
	    }
	    else if(j == targetPixY + 1){
	      dis+=pix_data.dist;
	    }
